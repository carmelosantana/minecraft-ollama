...
online-mode=false
white-list=false
motd=Ollama Plugin Test Server
view-distance=10
...
EOF
    ...
    print_status "Note: Make sure Ollama is running locally on port 11434"
...
show_status() {
    ...
    if curl -s http://localhost:11434/api/version &> /dev/null; then
        ...
        print_warning "Ollama API is not accessible. Make sure Ollama is running with 'ollama serve'"
    fi
}
...
show_network() {
    ...
    if curl -s http://localhost:11434/api/version &> /dev/null; then
        ...
        print_warning "Ollama API is not accessible. Start with 'ollama serve'"
    fi
}
...
case "${1:-help}" in
    ...
    players)
        ...
        ;;
    help|*)
        echo "Ollama Plugin Server Manager"
        echo ""
        ...
        echo "  status   - Show server and Ollama status"
